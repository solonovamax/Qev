...
package com.solostudios.qev.framework.old.main;
...
public class LevelCard {
    ...
    private static final Font                       levelFont            =
            new Font("Franklin Gothic Medium", Font.PLAIN, 84);
    ...
    private static final Logger                     logger               = LoggerFactory.getLogger(LevelCard.class);
    private static final Area                       circleCrop           = new Area(overlay);
    private static final BufferedImage              circleBuffer         =
            new BufferedImage(IMG_SIZE, IMG_SIZE, BufferedImage.TYPE_INT_ARGB);
    private static final Graphics2D                 circleBufferGraphics = circleBuffer.createGraphics();
    ...
    public static BufferedImage makeLevelCard(User user, int xp, int xpRemaining, int userLevel, int userPosition,
                                              int memberCount) throws MalformedURLException {
        long startTime = System.currentTimeMillis();
        
        BufferedImage avatar;
        try {
            avatar = ImageIO.read(
                    new URL("https://cdn.discordapp.com/avatars/" + user.getIdLong() + "/" + user.getAvatarId() +
                            ".png?size=256"));
        } catch (IOException e) {
        ...
        levelCardGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        ...
        levelCardGraphics.fillRect(LEVEL_BAR_X + 4, LEVEL_BAR_Y + 4, (int) ((LEVEL_BAR_WIDTH - 7) * levelPercent),
                                   LEVEL_BAR_HEIGHT - 7);
        ...
        String username = (metrics.stringWidth(user.getName()) > 900 ? user.getName().substring(0,
                                                                                                (int) (((double) 700 /
                                                                                                        metrics.stringWidth(
                                                                                                                user.getName())) *
                                                                                                       user.getName().length())) +
        ...
        levelCardGraphics.drawString(user.getAsTag().replace(user.getName(), ""),
                                     USERNAME_X + metrics.stringWidth(username), USERNAME_Y);
        ...
        levelCardGraphics.drawString(String.valueOf(userLevel), LEVEL_X + ((levelTextMetrics.stringWidth("Level") / 2) -
                                                                           (levelMetrics.stringWidth(
                                                                                   String.valueOf(userLevel)) / 2)),
                                     LEVEL_Y + (levelMetrics.getHeight()));
        ...
        levelCardGraphics.setFont(levelFont);
        levelCardGraphics.drawString(String.valueOf(userPosition), POSITION_X +
                                                                   ((levelTextMetrics.stringWidth("Rank") / 2) -
                                                                    (levelMetrics.stringWidth(
                                                                            String.valueOf(userPosition)) / 2)),
                                     POSITION_Y + (levelMetrics.getHeight()));
