...
public class Clear extends AbstractCommand {
	public Clear() {
		...
		this.setAliases("purge");
		this.setDescription("Clears messages from chat");
		this.setArguments(new JSONArray()
								  .put(new JSONObject()
											   .put("key", "amount")
											   .put("type", int.class)
											   .put("error", "Invalid amount of messages to clear!")));
		this.setUsage("clear {# of messages to clear}");
		this.setClientPermissions(Permission.MESSAGE_MANAGE);
		this.setUserPermissions(Permission.MESSAGE_MANAGE);
	}
	
	
	@Override
	public void run(MessageReceivedEvent event, ArgumentContainer args) throws IllegalInputException {
		event.getTextChannel().retrieveMessageById(event.getMessage().getId()).queue(msg -> msg.delete().queue(),
																					 error -> {
																					 });
